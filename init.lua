...
lakemaker.spawn_tnt = function(pos, entname)
	assert(table.indexof(all_tnt, entname) ~= -1, "attempting to spawn non-tnt")
	minetest.sound_play("lakemaker_ignite", {pos = pos, gain = 1.0, max_hear_distance = 16})
...
lakemaker.register_tnt = function(nodename, def)
		...
		mesecons = {
			effector = {
				action_on = function(pos, node)
					...
					lakemaker.spawn_tnt(pos, node.name)
	...
	function entity:on_step(dtime)
		...
		if self.timer > def.explodetime then -- boom!
			minetest.sound_play("lakemaker_explode", {pos = pos, gain = 0.8, max_hear_distance = 32})
...
lakemaker.register_tnt("lakemaker:lakemaker", {
	description = "Lakemaker",
	tiles = {"lakemaker_top.png", "lakemaker_bottom.png", "lakemaker_side.png"},
...
lakemaker.register_tnt("lakemaker:lakemakerx", {
	description = "Extreme Lakemaker",
	tiles = {"lakemaker_top.png", "lakemaker_bottom.png", "lakemaker_side.png"},
	on_explode = function(pos)
		on_explode_split(pos, 3, "lakemaker:lakemaker")
	end,
})

minetest.register_craft({
	output = "lakemaker:lakemakerx",
	recipe = {
		{"", "group:wood", ""},
		{"lakemaker:lakemaker", "lakemaker:lakemaker", "lakemaker:lakemaker"},
		{"", "default:steel_ingot", ""},
	}
})
...
minetest.register_alias("lakemaker:extreme_lakemaker", "lakemaker:lakemakerx")


if minetest.settings:get_bool("log_mods") then
	print("[lakemaker] Loaded")
end
